...
namespace
{
    namespace Problem
    {
        ...
        private:
            std::string _name;
            std::list<File *> _files; // Should store the pointer to the actual file so when we delete the file it should delete in the list
        ...
        public:
            ...
            {
                _files.remove_if([file](const File *f)
                                 { return f == file; });
            }
            ...
            void removeFolderByName(const std::string &name)
            {
                ...
                //     }
                // }
                auto it = std::find_if(_subFolders.begin(), _subFolders.end(),
                                       [&name](const Folder *f)
                                       { return f->getName() == name; });
            ...
            {
                _subFolders.remove_if([folder](const Folder *f)
                                      { return f == folder; });
            }
            ...
            int size() const
            {
                ...
                std::for_each(_subFolders.begin(), _subFolders.end(), [&size](const Folder *sf)
                              { size += sf->size(); });
            ...
            const std::list<Folder *> &getSubFolders() const
            {
                return _subFolders;
            }
        ...
        namespace Client
        {
            void clientCode(const Folder *folder)
            {
                ...
                std::cout << "Folder size: " << folder->getName() << ", size: " << folder->size() << "\n";
                std::list<File *> files;
    ...
    namespace CompositePattern
    {
        ...
        private:
            ...
            {
                _children.remove_if([file](const FileSystem *f)
                                    { return f == file; });
            }
